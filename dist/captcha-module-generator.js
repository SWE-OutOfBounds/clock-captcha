...
function setClock(canvas, time) {
    let hrRotation = (360 / 12) * (Math.trunc(time / 3600) % 12);
    let minRotation = (360 / 60) * (Math.trunc(time / 60) % 60);
    let secRotation = (360 / 60) * (time % 60);
    let v = null;
    const ctx = canvas ? canvas.getContext('2d') : null;
    // Read the SVG string using the fromString method
    // of Canvg
    if (ctx)
        v = canvg_1.Canvg.fromString(ctx, `<svg id="clock" xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 600 600">
    <g id="face">
...
function generateCaptcha() {
    let time = Math.trunc(Math.random() * (2 * 24 * 60 * 60));
    ...
    (0, stylist_1.inputDressing)(hoursInput);
    ...
    (0, stylist_1.inputDressing)(minutesInput);
